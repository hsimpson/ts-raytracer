...
export class ConstantMedium extends Hittable {
  ...
  public get boundary(): Hittable {
    return this._boundary;
  }

  public get negInvDensity(): number {
    return this._negInvDensity;
  }
  ...
  public hit(ray: Ray, tMin: number, tMax: number, rec: HitRecord): boolean {
    ...
    if (!this._boundary.hit(ray, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, rec1)) {
      return false;
    }

    if (!this._boundary.hit(ray, rec1.t + 0.0001, Number.POSITIVE_INFINITY, rec2)) {
      return false;
    }
    ...
    const rayLength = Vector.length(ray.direction);
