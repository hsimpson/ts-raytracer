...
const atomRead = (_store, atom2, ...params) => atom2.read(...params);
const atomWrite = (_store, atom2, ...params) => atom2.write(...params);
const atomOnInit = (store, atom2) => {
  var _a;
  return (_a = atom2.unstable_onInit) == null ? void 0 : _a.call(atom2, store);
};
const atomOnMount = (_store, atom2, setAtom) => {
  var _a;
  return (_a = atom2.onMount) == null ? void 0 : _a.call(atom2, setAtom);
};
const ensureAtomState = (store, atom2) => {
...
const readAtomState = (store, atom2) => {
  var _a;
  const buildingBlocks = getBuildingBlocks(store);
  const mountedMap = buildingBlocks[1];
  const invalidatedAtoms = buildingBlocks[2];
...
const invalidateDependents = (store, atom2) => {
  const buildingBlocks = getBuildingBlocks(store);
  const mountedMap = buildingBlocks[1];
  const invalidatedAtoms = buildingBlocks[2];
  const ensureAtomState2 = buildingBlocks[11];
...
const writeAtomState = (store, atom2, ...args) => {
  const buildingBlocks = getBuildingBlocks(store);
  const changedAtoms = buildingBlocks[3];
  const storeHooks = buildingBlocks[6];
...
const mountDependencies = (store, atom2) => {
  var _a;
  const buildingBlocks = getBuildingBlocks(store);
  const mountedMap = buildingBlocks[1];
  const changedAtoms = buildingBlocks[3];
...
const setAtomStateValueOrPromise = (store, atom2, valueOrPromise) => {
...
const storeSub = (store, atom2, listener) => {
  const buildingBlocks = getBuildingBlocks(store);
  const flushCallbacks2 = buildingBlocks[12];
  const mountAtom2 = buildingBlocks[18];
  const unmountAtom2 = buildingBlocks[19];
...
function getBuildingBlocks(store) {
  ...
  if ((__vite_import_meta_env__$3 ? "production" : void 0) !== "production" && !buildingBlocks) {
    throw new Error(
...
function defaultWrite(get, set2, arg) {
  return set2(
    this,
...
function Provider({
  ...
  const storeRef = reactExports.useRef(void 0);
...
class Ray {
  ...
  at(t) {
    return add(create$2(), this._orig, scale$1(create$2(), this._dir, t));
  }
}
...
function reflect(v, n) {
  return subtract(create$2(), v, scale$1(create$2(), n, 2 * dot(v, n)));
}
function refract(uv, n, etai_over_etat) {
  const cos_theta = dot(negate(create$2(), uv), n);
  const uvTheta = add(create$2(), uv, scale$1(create$2(), n, cos_theta));
  const r_out_parallel = scale$1(create$2(), uvTheta, etai_over_etat);
  const r_out_perp = scale$1(create$2(), n, -Math.sqrt(1 - lengthSquared(r_out_parallel)));
...
class Camera {
  ...
  getRay(s, t) {
    const rd = scale$1(create$2(), randomInUnitdisk(), this.lenseRadius);
    const vecU = scale$1(create$2(), this.u, rd[0]);
    const vecV = scale$1(create$2(), this.v, rd[1]);
    const offset = add(create$2(), vecU, vecV);
    const sHor = scale$1(create$2(), this.horizontal, s);
    const tVer = scale$1(create$2(), this.vertical, t);
    return new Ray(
      ...
      sub(
        ...
        sub(
          create$2(),
          add(create$2(), add(create$2(), this.lowerLeftCorner, sHor), tVer),
          this.lookFrom
        ),
        offset
      ),
...
class Transform {
  _objectToWorldMatrix = create$3();
  _worldToObjectMatrix = create$3();
  _rotationMatrix = create$3();
  _inverseRotationMatrix = create$3();
  _normalMatrix = create$3();
  _position = create$2();
  _rotation = create();
...
function requireAlea() {
  if (hasRequiredAlea) return alea$2.exports;
  hasRequiredAlea = 1;
  ...
  return alea$2.exports;
}
var aleaExports = requireAlea();
...
class MovingSphere extends Hittable {
  ...
  boundingBox(t0, t1) {
    ...
    const r = fromValues$1(this._radius, this._radius, this._radius);
    const box0 = new AABB(
...
class Triangle extends Hittable {
  ...
  hit(ray, tMin, tMax, rec) {
    ...
    const w = 1 - u - v;
    if (this.n0 && this.n1 && this.n2) {
      const n0 = scale$1(create$2(), this.n0, w);
      const n1 = scale$1(create$2(), this.n1, u);
      const n2 = scale$1(create$2(), this.n2, v);
...
async function load(url) {
  ...
  const defaultMaterial = new LambertianMaterial(fromValues$1(1, 1, 1));
  for (const material of materials) {
    const baseColor = material.pbrMetallicRoughness?.baseColorFactor ?? fromValues(0.5, 0.5, 0.5, 1);
    let mat;
...
class WebGPUComputePipline extends WebGPUPipelineBase {
  _options;
  _raytracingBuffers;
  _computeParamsUniformBuffer = new WebGPUBuffer();
  _computeCameraUniformBuffer = new WebGPUBuffer();
  _pixelBuffer = new WebGPUBuffer();
  _accumulationBuffer = new WebGPUBuffer();
  _primitivesBuffer = new WebGPUBuffer();
  _materialsBuffer = new WebGPUBuffer();
  _texturesBuffer = new WebGPUBuffer();
  constructor(options) {
    super();
...
class WebGPURenderPipeline extends WebGPUPipelineBase {
  _options;
  _vertexPositionBuffer = new WebGPUBuffer();
  // private _vertexColorBuffer = new WebGPUBuffer();
  _renderParamsUniformBuffer = new WebGPUBuffer();
  constructor(options) {
    super();
    this._options = options;
  }
...
function useResetAtom(anAtom, options) {
  ...
  const resetAtom = reactExports.useCallback(() => setAtom(RESET), [setAtom]);
  return resetAtom;
}
const raytracerProperties = atomWithReset({
  // 8K UHD
  // imageWidth: 7680,
  // imageHeight: 4320,
  // 4k UHD
...
const raytracerRunningProperties = atomWithReset({
  isRunning: false,
  stats: ""
});
const Canvas = () => {
  const canvasGPURef = React.useRef(null);
  ...
  const rayTracerGPURef = React.useRef(null);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "render-container", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "canvas-container", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      "canvas",
      {
        id: "canvas-gpu",
